...
function pat_article_social($atts)
{
	...
	if ( $site && !gps('txpreview') ) {
		...
		switch( strtolower($site) ) {
			...
			case 'facebook':
				$link = '<a href="http://www.facebook.com/sharer/sharer.php?u='.$url.'&amp;t='.urlencode( title(array()) ).'" title="'.$tooltip.'" class="social-link'.($class ? ' '.$class : '').'" target="_blank">'.($icon ? '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" class="facebook-icon" x="0" y="0" width="'.$width.'" height="'.$height.'" viewBox="0 0 11.861 23.303"><path d="M11.861,8.323H7.594V6.243c0,0-0.239-1.978,1.144-1.978c1.563,0,2.811,0,2.811,0V0H6.763c0,0-4.005-0.017-4.005,4.005c0,0.864-0.004,2.437-0.01,4.318H0v3.434h2.741c-0.016,5.46-0.035,11.545-0.035,11.545h4.888V11.757h3.226L11.861,8.323z"/></svg>' : '').'<b>'.$title.'</b>'.($count && $zero ? '  <span>'._pat_article_social_get_content( $thisarticle['thisid'].'-'.$site, urlencode( permlink(array()) ), '_pat_article_social_get_facebook', $delay, $zero ).'</span>' : '').($fallback ? '<strong>F</strong>' : '').'</a>';
			break;


			case 'google':
				$link = '<a href="https://plus.google.com/share?url='.$url.'" title="'.$tooltip.'" class="social-link'.($class ? ' '.$class : '').'" target="_blank">'.($icon ? '<svg xmlns="http://www.w3.org/2000/svg" version="1" class="gplus-icon" width="'.$width.'" height="'.$height.'" viewBox="0 0 491 305" preserveAspectRatio="xMidYMid meet"><g transform="translate(0.000000,305.000000) scale(0.100000,-0.100000)" fill="#000000" stroke="none"><path d="M1340 3039c-241-28-516-138-716-287-116-86-263-235-343-348-71-101-167-290-205-408-55-167-70-266-70-471 0-157 4-207 22-291 133-618 595-1080 1204-1204 131-27 325-36 463-21 367 40 640 164 876 400 320 319 460 765 409 1300l-12 131-724 0-724 0 2-307 3-308 412-3 411-2-14-62c-41-173-170-341-336-437-240-136-584-152-832-37-247 114-431 327-513 593-24 79-27 104-27 243-1 121 3 170 18 224 79 292 278 525 541 633 118 48 210 65 345 65 219 0 389-60 538-189l49-41 218 218c120 120 215 223 211 228-13 21-191 149-271 195-269 156-609 223-935 186zM3940 1995l0-275-275 0-275 0 0-195 0-195 275 0 275 0 0-275 0-275 205 0 205 0 0 275 0 275 275 0 275 0 0 200 0 200-275 0-275 0 0 270 0 270-205 0-205 0 0-275z"/></g></svg>' : '').'<b>'.$title.'</b>'.($count ? _pat_article_social_get_content( $thisarticle['thisid'].'-'.$site, urlencode( permlink(array()) ), '_pat_article_social_get_google', $delay, $zero ) : '').($fallback ? '<strong>G+</strong>' : '').'</a>';
			break;
